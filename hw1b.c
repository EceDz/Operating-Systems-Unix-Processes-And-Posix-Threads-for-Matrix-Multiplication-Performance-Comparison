...
hw1b.c - Multithreaded Matrix Multiplication with Mutual Exclusion
Each thread computes one row of the result matrix using mutex 
 */

#include <stdio.h>
#include <stdlib.h>
#include <pthread.h> // For thread creation, join, mutex
#include <sys/time.h>
#include <time.h>
...
// Mutex for protecting access to matrices
...
void *calculate_row(void *arg) {
    ...
    for (int j = 0; j < N; j++) {
        int sum = 0;
        for (int k = 0; k < N; k++) {
            // Lock mutex before accessing shared matrices
            pthread_mutex_lock(&mymutex);
            sum += A[row][k] * B[k][j];
            pthread_mutex_unlock(&mymutex);
        }
        
        // Lock mutex before writing to result
        pthread_mutex_lock(&mymutex);
        C[row][j] = sum;
        pthread_mutex_unlock(&mymutex);
    }
...
int main(int argc, char *argv[]) {
    ...
    printf("\nMultithreaded Matrix Multiplication with Mutex\n");
    ...
    for (int i = 0; i < N; i++) {
        ...
        mydata->row = i;
        //Each created thread runs according to the mutex restrictions in calculate_row
